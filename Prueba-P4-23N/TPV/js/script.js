...
function insertarFacturacion(vProductos) {
    console.log("entro en insertarFacturacion");
    console.log(vProductos);

    let infoCLiente = prompt("Intoduza su nombre y apellidos, el número de teléfono y un email");
    $.ajax({
        url: "http://moralo.atwebpages.com/menuAjax/productos3/insertarFacturacion.php",
        type: "POST",
        data: {
            // sintaxis: variablePHP : variableJs
            total: parseInt(precioTotal.textContent),
            productos: vProductos.toString(),
...
function mostrarPDF() {
    ...
    insertarFacturacion(vProductos);
    ...
    win.document.write("</table>");
    win.document.write("Total: " + precioTotal.textContent); // contenidos dentro del body
